		...
		final String postgreSQLURL = "jdbc:postgresql://localhost/test?user=transaktionsmanagement&password=Apfelkuchen?!";
			...
			for (String url : connections) {
				...
				try (final Connection conn = DriverManager.getConnection(url)) {
					...
					BankTransferTransaktionThread ab = new BankTransferTransaktionThread(conn, url, "A", "B", BigDecimal.valueOf(100));
					BankTransferTransaktionThread bc = new BankTransferTransaktionThread(conn, url, "B", "C", BigDecimal.valueOf(100));
					BankTransferTransaktionThread ca = new BankTransferTransaktionThread(conn, url, "C", "A", BigDecimal.valueOf(100));
					...
					bc.join();
					ca.join();
					try(final Connection conn2 = DriverManager.getConnection(url)){
						listKunden(conn2);
					}catch (Exception e) {
						System.out.println("Wir wollten doch nur die Ergebnisse printen.");
					}
